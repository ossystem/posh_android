...
/* package */ class UuidHelper {
	...
	/* package */ static void assignCustomUuids(final Intent intent) {
		...
		Parcelable[] uuids = intent.getParcelableArrayExtra(DfuBaseService.EXTRA_CUSTOM_UUIDS_FOR_LOAD_IMAGE);
		if (uuids != null && uuids.length == 4) {
			FileLoadImpl.DEFAULT_UART_SERVICE_UUID	= uuids[0] != null ? ((ParcelUuid) uuids[0]).getUuid() : FileLoadImpl.DEFAULT_UART_SERVICE_UUID;
			FileLoadImpl.DEFAULT_UART_CONTROL_UUID	= uuids[1] != null ? ((ParcelUuid) uuids[1]).getUuid() : FileLoadImpl.DEFAULT_UART_CONTROL_UUID;
			FileLoadImpl.DEFAULT_UART_RX_UUID       = uuids[2] != null ? ((ParcelUuid) uuids[2]).getUuid() : FileLoadImpl.DEFAULT_UART_RX_UUID;
			FileLoadImpl.DEFAULT_UART_TX_UUID      	= uuids[3] != null ? ((ParcelUuid) uuids[3]).getUuid() : FileLoadImpl.DEFAULT_UART_TX_UUID;
		}
